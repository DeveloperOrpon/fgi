...
class InputPasswordScreen extends StatefulWidget {
  final String email;
  const InputPasswordScreen({Key? key, required this.email}) : super(key: key);
            ...
            children: [
              ...
              SizedBox(
                child: Pinput(
                  ...
                  onCompleted: (pin) {
                    startLoading('');
                    authController.checkOTP(controller.text.trim(), widget.email!, context).then((value){
                      EasyLoading.dismiss();
                      if(value){
                        Get.to( ChangePasswordScreen(email: widget.email!),transition: Transition.fadeIn);
                      }else{
